...
<div class="container mt-5">
    ...
    <div class="card border-info shadow-lg rounded-lg">
        <div class="card-body">
            <h3 class="card-title text-center text-info mb-4">Thông tin tra cứu đơn hàng #{{ $order['id'] }}</h3>
            <div class="row">
                <div class="col-md-4 mb-4">
                    <img src="http://localhost:8000/{{ $order['product_image'] }}" alt="Ảnh sản phẩm" class="img-fluid rounded shadow-sm">
                </div>

                <div class="col-md-8">
                    <div class="order-info">
                        <h4 class="product-name text-primary">{{ $order['product_name'] }}</h4>
                        <p><strong>SKU:</strong> {{ $order['sku'] }}</p>
                        <p><strong>Kích Cỡ:</strong> {{ $order['size_name'] }}</p>
                        <p><strong>Màu Sắc:</strong> {{ $order['color_name'] }}</p>
                        <p><strong>Giá:</strong> {{ number_format($order['price'], 0, ',', '.') }} VND</p>
                        <p><strong>Số Lượng:</strong> {{ $order['quantity'] }}</p>
                        <p><strong>Tổng Tiền:</strong> {{ number_format($order['total_price'], 0, ',', '.') }} VND</p>
                        
                        <div class="payment-status-container mb-4">
                            <p class="payment-status {{ strtolower($order['payment_status']) }}"><strong>Trạng Thái Thanh Toán:</strong> {{ $order['payment_status'] }}</p>
                        </div>

                        <p class="payment-method {{ strtolower($order['payment_method']) }}"><strong>Phương Thức Thanh Toán:</strong> {{ $order['payment_method'] }}</p>
